...
float setPWMWithFlutter(int data) {
  switch(data) {
    case 1: //FORWARD
        return 8; //max rpm is 300 ish
        break;
    case 3: //BACKWARDS
        return -8;
        break;
...
void loop() {
  ...
  if (central) {
    ...
    if (central.connected()) {
      ...
      if (customCharacteristic.written()) {
        ...
        addPWM = setPWMWithFlutter(command);      // set target angle according to button press, and pass int, not char* 
  ...
  if ((tilt_angle >= -30)&&(tilt_angle <= 30)) {
    ...
    pwm1 = avePWM + difPWM/2 + addPWM;
    pwm2 = avePWM - difPWM/2 + addPWM;
