// SHOWTIME: goes up 15 deg ramp and down with minimal turning AT A HIGH VOLTAGE
// 11.4V on rails and 70db 
// works for push test
...
float setDriveWithFlutter(int data) {
  switch (data) {
    ...
    case 5:  //A OR STOP
      return 0.0;
      break;
    case 8: //up 5 deg ramp
      return 0.4;
      break;
    case 9: // down 5 deg ramp
      return -0.2;
      break;
    case 11: // up 10 deg ramp
      return 0.3;
      break;
    case 12: // down 10 deg ramp
      return -0.1;
      break;
    case 13: //up 15 deg ramp
      return 0.2;
      break;
    case 14: //down 15 deg ramp
      return -0.1;
      break;
    default:     //STOP
...
float setTargetAngleWithFlutter(int data) {
  switch (data) {
    case 8: // up 5 deg ramp
      return 5.9;
      break;
    ...
    if (central) {
      ...
      if (central.connected()) {
        ...
        if (customCharacteristic.written()) {
          ...
          rampAngle = setTargetAngleWithFlutter(command);
...
void setup() {
  ...
  drivePID = {
    ...
    .OutputMax = 18.0, //max target angle
    .ErrorIntMax = 1.33
  };
  ...
  turnPID = {
    ...
    .Kd = 0,
    .TargetDefault = 0, // default target speed
    .OutputMax = 200.0, //max target angle
    .ErrorIntMax = 19.0
  };
...
void loop() {
  ...
  if (currentTime - lastLoopTime >= LOOP_TIME_MS) { //LOOP_TIME_MS currently set to 10000 microseconds (10 ms)
    ...
    if (tilt_angle >= -40.0 && tilt_angle <= 40.0) {
      turnPID.Actual = difRPM;
      updatePID(turnPID, dt_seconds);
    ...
    if (driveCount >= 5) {
      ...
      anglePID.Target = drivePID.Output + anglePID.TargetDefault + rampAngle; //clamped drivepid.output to 3 deg
      ...
      // if(driveCommand != 0 && abs((driveEnc1 + driveEnc2)/2.0f) < 3000){
