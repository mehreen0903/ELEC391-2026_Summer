// ─────────────────────────────────────────────────────────────────────
//  CustomIMU.cpp — BMI270 bare-register driver
//  See CustomIMU.h for setup instructions.
// ─────────────────────────────────────────────────────────────────────
#include "CustomIMU.h"
//#include "bmi270_config_file.h"   // <-- copy from the Arduino library
...
CustomIMU_Class IMU;              // the global object your code uses
 
// ═════════════════════════════════════════════════════════════════════
//  I2C HELPERS
// ═════════════════════════════════════════════════════════════════════
 
void CustomIMU_Class::writeReg(uint8_t reg, uint8_t val) {
...
uint8_t CustomIMU_Class::readReg(uint8_t reg) {
  wire1.beginTransmission(ADDR);
  wire1.write(reg);
  wire1.endTransmission(false);          // repeated-start (keep bus)
...
void CustomIMU_Class::readRegs(uint8_t reg, uint8_t *buf, uint8_t len) {
  wire1.beginTransmission(ADDR);
  wire1.write(reg);
...
// ═════════════════════════════════════════════════════════════════════
//  CONFIG-FILE UPLOAD
//  The BMI270 is unusual: it is a blank slate at power-on and will not
//  output any sensor data until its 8 KB firmware blob has been written
//  over I2C. The Arduino wire1 buffer is 32 bytes, so we write it in
//  16-byte chunks (16 data + 1 register address = 17 bytes ≤ 32).
// ═════════════════════════════════════════════════════════════════════
 
bool CustomIMU_Class::uploadConfigFile() {
  // 1. Disable advanced power-save so the init registers are accessible.
  writeReg(REG_PWR_CONF, 0x00);
  delay(1);
 
  // 2. Halt the init engine before writing.
  writeReg(REG_INIT_CTRL, 0x00);
 
  // 3. Burst-write the config blob in 16-byte chunks.
  //    Before each chunk we write the word-address (byte_offset / 2)
  //    into INIT_ADDR_0 (bits [3:0]) and INIT_ADDR_1 (bits [11:4]).
  constexpr uint8_t  CHUNK = 16;
  const uint16_t     total = (uint16_t)sizeof(bmi270_config_file);
 
  for (uint16_t i = 0; i < total; i += CHUNK) {
    uint16_t wordAddr = i / 2;
    writeReg(REG_INIT_ADDR_0, (uint8_t)(wordAddr & 0x0F));
    writeReg(REG_INIT_ADDR_1, (uint8_t)(wordAddr >> 4));
 
    uint16_t n = (i + CHUNK <= total) ? CHUNK : (total - i);
    wire1.beginTransmission(ADDR);
    wire1.write(REG_INIT_DATA);
    for (uint16_t j = 0; j < n; j++)
      wire1.write(bmi270_config_file[i + j]);
    wire1.endTransmission();
  }
 
  // 4. Trigger initialization and wait for it to finish.
  writeReg(REG_INIT_CTRL, 0x01);
  delay(20);  // datasheet minimum; increase to 140 ms if init keeps failing
 
  // 5. Confirm success: INTERNAL_STATUS bits [3:0] must equal 0x01.
  uint8_t status = readReg(REG_INTERNAL_STATUS) & 0x0F;
  return (status == 0x01);
}
...
// ── ODR configuration helpers ─────────────────────────────────────────
// Call these BEFORE begin() if you want a non-default rate.
// ODR byte map: 25→0x06, 50→0x07, 100→0x08, 200→0x09, 400→0x0A
void CustomIMU_Class::setAccelODR(uint16_t hz) {
  if      (hz <= 25)  _accOdrByte = 0x06;
  else if (hz <= 50)  _accOdrByte = 0x07;
  else if (hz <= 100) _accOdrByte = 0x08;
  else if (hz <= 200) _accOdrByte = 0x09;
  else                _accOdrByte = 0x0A; // 400 Hz
}
void CustomIMU_Class::setGyroODR(uint16_t hz) {
  if      (hz <= 25)  _gyrOdrByte = 0x06;
  else if (hz <= 50)  _gyrOdrByte = 0x07;
  else if (hz <= 100) _gyrOdrByte = 0x08;
  else if (hz <= 200) _gyrOdrByte = 0x09;
  else                _gyrOdrByte = 0x0A;
}
 
// ── begin() ──────────────────────────────────────────────────────────
bool CustomIMU_Class::begin() {
  wire1.begin();
  wire1.setClock(400000);              // Fast-Mode (400 kHz) — cuts upload time
 
  // Verify we can talk to a BMI270 (chip-ID is fixed at 0x24).
  if (readReg(REG_CHIP_ID) != 0x24) return false;
 
  // Soft-reset: clears all registers to power-on defaults.
  writeReg(REG_CMD, 0xB6);
  delay(10);
 
  // The BMI270 MUST have its firmware loaded — no config → no data.
  if (!uploadConfigFile()) return false;
 
  // ── Power on acc + gyro ───────────────────────────────────────────
  //  PWR_CTRL: bit3=acc enable, bit2=gyr enable  (bit1=aux, not used)
  writeReg(REG_PWR_CTRL, 0x0C);
  delay(5);
 
  // ── Accelerometer config ──────────────────────────────────────────
  //  ACC_CONF bit layout:
  //    bit  7   : filter_perf   (1 = performance mode ← we want this)
  //    bits 6:4 : acc_bwp       (0x2 = Normal filter)
  //    bits 3:0 : acc_odr       (0x09 = 200 Hz default)
  //
  //  0x80 | 0x20 | _accOdrByte  →  perf=1, bwp=normal, odr=200Hz = 0xA9
  writeReg(REG_ACC_CONF,  (uint8_t)(0x80 | 0x20 | _accOdrByte));
  writeReg(REG_ACC_RANGE, 0x00);         // ±2 g  (16384 LSB/g)
  _accelScale = 2.0f / 32768.0f;         // g per raw LSB
 
  // ── Gyroscope config ──────────────────────────────────────────────
  //  GYR_CONF bit layout:
  //    bit  7   : gyr_filter_perf  (1 = performance)
  //    bit  6   : gyr_noise_perf   (1 = performance)
  //    bits 5:4 : gyr_bwp          (0x2 = Normal filter  → 0x20)
  //    bits 3:0 : gyr_odr          (0x09 = 200 Hz default)
  //
  //  0x80 | 0x40 | 0x20 | _gyrOdrByte  = 0xE9 at 200 Hz
  writeReg(REG_GYR_CONF,  (uint8_t)(0x80 | 0x40 | 0x20 | _gyrOdrByte));
  writeReg(REG_GYR_RANGE, 0x00);         // ±2000 °/s  (16.384 LSB/°/s)
  _gyroScale = 2000.0f / 32768.0f;       // °/s per raw LSB
 
  // Exit low-power mode; keep FIFO self-wakeup enabled.
  writeReg(REG_PWR_CONF, 0x02);
  delay(1);
...
//   bit 7 → drdy_acc  (set when a new accelerometer frame is in the registers)
//   bit 6 → drdy_gyr  (set when a new gyroscope frame is in the registers)
// The bit clears automatically when you call readAcceleration / readGyroscope.
 
bool CustomIMU_Class::accelerationAvailable() {
  return (readReg(REG_STATUS) & 0x80) != 0;
}
 
bool CustomIMU_Class::gyroscopeAvailable() {
  return (readReg(REG_STATUS) & 0x40) != 0;
}
 
// ── Sensor reads ──────────────────────────────────────────────────────
// Each axis is a 16-bit two's-complement signed integer, little-endian.
// Cast to int16_t BEFORE multiplying so sign extension works correctly.
 
bool CustomIMU_Class::readAcceleration(float &x, float &y, float &z) {
  uint8_t buf[6];
  readRegs(REG_ACC_X_LSB, buf, 6);
  x = (int16_t)((buf[1] << 8) | buf[0]) * _accelScale;
  y = (int16_t)((buf[3] << 8) | buf[2]) * _accelScale;
  z = (int16_t)((buf[5] << 8) | buf[4]) * _accelScale;
  return true;
}
 
bool CustomIMU_Class::readGyroscope(float &x, float &y, float &z) {
  uint8_t buf[6];
  readRegs(REG_GYR_X_LSB, buf, 6);
  x = (int16_t)((buf[1] << 8) | buf[0]) * _gyroScale;
  y = (int16_t)((buf[3] << 8) | buf[2]) * _gyroScale;
  z = (int16_t)((buf[5] << 8) | buf[4]) * _gyroScale;
  return true;
}
