...
//dt variables
const unsigned long LOOP_TIME_MS = 5; //target loop time = 5 ms = 2000Hz
...
float calcRPM(volatile long &encCount, long &prevCount, unsigned long &prevTime, float dt_seconds) {
...
void loop() {
  ...
  const float dt_seconds = (currentTime - lastLoopTime) / 1000000.0; // Convert to seconds
  ...
  float rpm1 = calcRPM(encCount1, prevCount1, prevTime1, dt_seconds);
  float rpm2 = calcRPM(encCount2, prevCount2, prevTime2, dt_seconds);
  aveRPM = (rpm1 + rpm2) * 0.5;
  difRPM = rpm1 - rpm2;
  drivePID.Actual = aveRPM;  //we changed it so that this is now in RPS!!
  updatePID(drivePID, dt_seconds);
