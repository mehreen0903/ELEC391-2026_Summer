...
// 50 cm run +/- 3cm: 11.2V - 11.36V
// Turns well +/- 3 deg: 11.08V
...
float setDriveWithFlutter(int data) {
  switch (data) {
    case 1:        //FORWARD
      return 0.9;  //max rpm is 300 ish
      break;
    case 3:  //BACKWARDS
      return -0.9; //bc of unequal dbr
      break;
    case 2:  //LEFT
      return 0.0;
      break;
...
void setup() {
  ...
  anglePID = {
    ...
    .Ki = 8,//7.3, //6
    .Kd = 0.15,//0.06, //0.08
    .TargetDefault = 0.9, // target angle
    .OutputMax = 100,
  ...
  turnPID = {
    ...
    .Kd = 0.0,
    .TargetDefault = 0, // default target speed
    .OutputMax = 30.0, //max target angle
    .ErrorIntMax = 15.0
  };
...
void loop() {
  ...
  if (currentTime - lastLoopTime >= LOOP_TIME_MS) { //LOOP_TIME_MS currently set to 10000 microseconds (10 ms)
    ...
    if(turnPWM != 0 && ((turnAngle < 39) && (-turnAngle < 41))){
    ...
    if (driveCount >= 5) {
      ...
      if(driveCommand != 0 && ((aveEnc < 2900) && (-aveEnc < 3100))){
        resetFlag = 1;
